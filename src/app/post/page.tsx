  ...
  const selectTransport = (imgSrc: JSX.Element) => {
    setIsImageIdx((prevState) => {
      ...
      const selectedIndex = prevState.findIndex(
        (item) =>
          item.imgsrc.type === imgSrc.type &&
          item.imgsrc.props.fillColor === imgSrc.props.fillColor
      );
    ...
    <div className="">
      ...
      <div className="w-[90%] sm-700:w-[50%] mx-auto">
        <div className="flex items-center mt-[5rem]">
          ...
          ></button>
          <button
            className="ml-auto flex bg-[#FB3463] text-white text-[1.6rem] font-semibold rounded-[1rem] px-[2.5rem] py-[0.5rem]"
            onClick={addPost}
          >
            올리기
          </button>
        ...
        {typeof window !== "undefined" && window.innerWidth > 600 ? (
          <div className="max-w-full h-[32rem] shadowall rounded-[1rem] flex mx-auto mt-[2rem]">
            <div
              className={`w-full max-w-[4.7rem] min-w-[1.7rem] h-full bg-[${bgColor}] rounded-l-[1rem] shadowall`}
            ></div>
            <div className="w-full mt-[5rem] relative">
              <div className="flex justify-center">
                <div>
                  ...
                  <div className="w-[8rem] 2xl:w-[18rem] sm-1200:sm-1550:sm:w-[8rem] h-[3.6rem] px-[2rem] shadowall rounded-[0.8rem] flex">
                    ...
                    <button
                      className="ml-auto"
                      onClick={() => searchCountry(inputValue1)}
                    >
                  ...
                  {isTransport ? (
                    <div className="w-[6rem] h-[31rem] absolute z-10 bg-white shadowall rounded-[3rem] flex items-center justify-center mt-[2rem] flex-col space-y-1">
                      {isImageIdx.slice(0, 5).map((item: any, index) => (
                        <div
                          key={index}
                          className="w-[6rem] h-[6rem] p-[1rem]"
                ...
                <div className="ml-[5rem]">
                  ...
                  <div className="w-[11rem] 2xl:w-[18rem] sm-1550:w-[11rem] h-[3.6rem] px-[2rem] shadowall rounded-[0.8rem] flex mt-4">
                    ...
                    <button
                      className="ml-auto"
                      onClick={() => searchCountry1(inputValue2)}
                    >
                      <Image src={searchicon} alt="" />
                    </button>
                  </div>
              ...
              >
                ...
                <span className="w-full max-w-[15em] min-w-[10rem]">DATE</span>
              ...
              >
                ...
                <div className="w-full max-w-[8rem] min-w-[2rem] flex text-[1.6rem]">
                  ...
                  <span className="mx-[1rem] mt-[0.5rem]">
                    {passengerCount}
                  </span>
                  <button
                    className="text-[#FB3463] flex text-[2rem]"
            ...
            <div
              className={`w-full max-w-[35rem] min-w-[20rem] h-full bg-[${bgColor}] rounded-r-[1rem] shadowall flex ml-auto`}
            >
              ...
              <label
                className="w-full h-full flex cursor-pointer p-[2rem]"
                htmlFor="input-file"
              >
                {thumbnailPreview === null ? (
                  <div className="flex flex-col justify-center m-auto w-full max-w-[30rem] h-full max-[25rem] rounded-[1rem] border-2 border-dashed border-white">
                    ...
                    <span className="text-[1.4rem] font-bold text-white text-center">
                      대표사진을 등록해주세요
                    </span>
                  </div>
                ) : (
                  <div className="flex flex-col m-auto">
                    <Image
                      className=" w-[30rem] h-[25rem] rounded-[1rem] object-cover"
          ...
          <div className="w-full h-[11rem] border border-[#D9D9D9] rounded-[1rem] flex mx-auto mt-[2rem]">
            <div
              className={`w-[1.4rem] h-full bg-[${bgColor}] rounded-l-[1rem]`}
            ></div>
            <div className="w-fit relative">
              <div className="flex justify-center">
                <div>
                  <h1 className="h-[3rem] mt-[1.3rem] text-[2rem] text-[#292929] font-extrabold font-akira">
                    {result?.result.isoAlp3}
                  </h1>
                    ...
                    <div className="w-[2rem] h-[13rem] absolute z-10 bg-white shadowall rounded-[3rem] flex items-center justify-center mt-[2rem] flex-col space-y-2">
                      {isImageIdx.slice(0, 5).map((item: any, index) => (
                        <div
                          key={index}
                          onClick={() => selectTransport(item.imgsrc)}
                          className="flex items-center justify-center py-[0.1rem] px-[1rem]"
                        >
                          {/* SVG 이미지에 크기 조정 */}
              ...
              <div
                className={`flex text-[0.5rem] ml-[1rem] font-extrabold font-akira`}
                style={{ color: bgColor || "inherit" }}
              >
              ...
              >
                <span className="w-[5rem] flex mt-[0.3rem]">
                  {memberData?.result.nickName}
                </span>
            ...
            <div
              className={`w-fit h-full bg-[${bgColor}] rounded-r-[1rem] flex ml-auto`}
            >
              <div className="absolute">
              ...
              <label
                className="w-fit h-full flex cursor-pointer p-[0.7rem]"
                htmlFor="input-file"
              >
                {thumbnailPreview === null ? (
                  <div className="flex flex-col justify-center m-auto w-[8.5rem] h-[9.5rem] rounded-[1rem] border-2 border-dashed border-white">
                    <Image
                      ...
                      height={18}
                    />
                    <span className="text-[0.55rem] font-bold text-white text-center">
                      대표사진 등록하기
                    </span>
                  </div>
                ) : (
                  <div className="flex flex-col m-auto">
                    <Image
                      className="w-[8.5rem] h-[9.5rem] rounded-[1rem] object-cover"
                      src={thumbnailPreview}
                      width={85}
                      height={95}
                      alt=""
        ...
        <>
          ...
          <div className="w-full h-[13rem] shadowall mt-[0.5rem] mb-[10rem] rounded-[0.8rem]">
            <input
              className={`text-[1.6rem] font-medium w-[50rem] outline-none ${typeof window !== "undefined" && window.innerWidth > 600 ? "ml-[6rem]" : "ml-[2rem]"} mt-[3.4rem]`}
              type="text"
            ...
            />
            <div
              className={`flex ${typeof window !== "undefined" && window.innerWidth > 600 ? "ml-[6rem]" : "ml-[2rem]"} mt-[1rem]`}
            >
              {tags.map((tag, index) => (
                <span
                  key={index}
                  className="flex items-center bg-[#fa3463] text-white rounded-[1.6rem] text-[1.6rem] px-[0.8rem] py-[0.4rem] mr-2"
                >
                  {tag}
                  <button
