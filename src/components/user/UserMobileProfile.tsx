  ...
  return (
    ...
    <div className="relative w-full">
      <div className="absolute top-[-240px] left-1/2 transform -translate-x-1/2 w-full h-[240px] px-8 py-4 flex flex-col items-center">
        ...
        {/* blogIntroduce 영역에 조건부로 pt-[10px] 적용 */}
        <h1 className="text-4xl text-white font-bold mt-4">{blogName}</h1>
        <span className={`text-xl text-white text-gray-600 mt-2 ${!userInfo?.memberId ? 'pt-[10px]' : ''}`}>
          {blogIntroduce}
        </span>
        ...
        {/* 팔로워/팔로잉 영역에 조건부로 pt-[10px] 적용 */}
        <div className={`flex px-4 gap-12 text-center mt-4 ${!userInfo?.memberId ? 'pt-[10px]' : ''}`}>
