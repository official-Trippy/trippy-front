  ...
  return (
    <form onSubmit={handleSubmit} className="w-[80%] mx-auto mt-[9.5rem]">
      <div className='sign-up-info'>로그인 정보를 설정해주세요</div>
        <label htmlFor="email" className="sign-up-info block mt-[6.9rem]">이메일</label>
        <div
       ...
       <input
        ...
        <button
            ...
            disabled={!emailValid || duplicateMessage !== '사용 가능한 이메일입니다.'}
            className={`${duplicateMessage === '사용 가능한 이메일입니다.' ? 'bg-black text-white hover:bg-gray-900 focus:outline-none focus:bg-gray-900' : 'bg-gray-400 text-white cursor-not-allowed'} w-[8.6rem] h-[3.5rem] my-auto rounded-lg`} style={{fontSize: '1.6rem'}}
        >
            인증하기
        </button>
        </div>
      ...
      <div className="mt-[6rem]">
        ...
        <input type="password" id="password" value={password} onChange={handlePasswordChange} placeholder="영어, 숫자, 특수 기호를 포함한 8~14자리" className="w-full px-4 py-2 mt-[2.5rem] mb-2 h-[6rem] rounded-xl border border-gray-300 focus:border-[#FB3463] focus:outline-none" style={{ background: 'var(--4, #F5F5F5)', fontSize: '1.5rem'  }}/>
        <div className='h-[1.7rem]'>
      ...
      <div className="mt-[6rem]">
        ...
        <input type="password" id="confirmPassword" value={confirmPassword} onChange={handleConfirmPasswordChange} placeholder="영어, 숫자, 특수 기호를 포함한 8~14자리" className="w-full px-4 py-2  mt-[2.5rem] mb-2 h-[6rem] rounded-xl border border-gray-300 focus:border-[#FB3463] focus:outline-none" style={{ background: 'var(--4, #F5F5F5)', fontSize: '1.5rem'  }} />
        <div className='h-[1.7rem]'>
      ...
      <div className="mt-[5.8rem] flex items-center" style={{fontSize: '1.5rem'  }}>
